...
export default function ToggleThemeMenu({ themeName, toggleTheme, t, className, style, size, widthBlock }) {
    useEffect(() => {
        document.getElementsByTagName('HTML')[0].className = `${themeName} ${widthBlock ? 'double' : 'single'}`;
    }, [themeName])
